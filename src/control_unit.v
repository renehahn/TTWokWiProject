...
module control_unit #(
    ...
    parameter CELL_W = 8,          // data cell width (bits)
    parameter TAPE_ADDR_W = 3      // data tape address width (8 cells)
)(
    input  wire                      clk_i,
    input  wire                      rst_i,
    ...
    reg uart_rx_ready, uart_rx_ready_next;
    ...
    generate
        if (TAPE_ADDR_W >= 4) begin : gen_extend_arg
            assign arg_dp_signed = $signed({{(TAPE_ADDR_W-4){arg5[4]}}, arg5});
        end else begin : gen_truncate_arg
            assign arg_dp_signed = $signed(arg5[TAPE_ADDR_W:0]);
        end
    endgenerate
    ...
    always @(posedge clk_i or negedge rst_i) begin
        if (!rst_i) begin
            ...
            uart_rx_ready <= uart_rx_ready_next;
        end
    end
    ...
    // UART RX
    assign uart_rx_ready_o = uart_rx_ready;
